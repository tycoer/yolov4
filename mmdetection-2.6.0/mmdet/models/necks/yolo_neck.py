...
class FuseStage(nn.Module):
    def __init__(self, in_channels,cfg,is_reversal = False,):
...
def MakeNConv(in_channels,out_channels,cfg,n):
...
class YOLOV4Neck(nn.Module):
    ...
    def __init__(self,
        ...
        layer_list = [
            OrderedDict([
                ('head_body0_0', MakeNConv(1024, 512,cfg, n = 3)),
                ('spp', SpatialPyramidPooling()),
                ('head_body0_1', MakeNConv(2048, 512,cfg, n = 3)), ]
            ),
            OrderedDict([
                ('fuse_0', FuseStage(512,cfg)),
                ('head_body1_0', MakeNConv(512, 256,cfg, 5))]
            ),

            OrderedDict([
                ('fuse_1', FuseStage(256,cfg)),
                ('head_body2_1', MakeNConv(256, 128,cfg, 5))
            ]),  # 输出0

            OrderedDict([
                ('fuse_2', FuseStage(128, cfg, is_reversal=True)),
                ('head_body1_1', MakeNConv(512,256, cfg, 5))]
            ),  # 输出1

            OrderedDict([
                ('fuse_3', FuseStage(256, cfg, is_reversal=True)),
                ('head_body0_2', MakeNConv(1024,512,cfg, 5))]
            ),  # 输出2
        ]
